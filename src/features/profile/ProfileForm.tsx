...
import { Container, Title, Card, Stack, Button, Group, Text } from '@mantine/core';
import { useAuthStore } from '../../shared/stores/authStore';
import { useFormValidation, ValidationSchema } from '../../shared/hooks/useFormValidation';
import { useProfileSave } from '../../shared/hooks/useApiCall';
import { ProfileData, ProfileFormProps, VALIDATION_LIMITS } from '../../shared/types/profile';
...
const profileValidationSchema: ValidationSchema = {
  name: [
    {
      validate: (value: unknown, allValues: Record<string, unknown>) => typeof value === 'string' && value.trim().length >= VALIDATION_LIMITS.name.minLength,
      message: `Le nom doit contenir au moins ${VALIDATION_LIMITS.name.minLength} caractères`,
    },
  ],
  email: [
    {
      validate: (value: unknown, allValues: Record<string, unknown>) => typeof value === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value),
      message: 'Veuillez entrer une adresse email valide',
    },
  ],
  age: [
    {
      validate: (value: unknown, allValues: Record<string, unknown>) => typeof value === 'number' && value >= VALIDATION_LIMITS.age.min && value <= VALIDATION_LIMITS.age.max,
      message: `L'âge doit être entre ${VALIDATION_LIMITS.age.min} et ${VALIDATION_LIMITS.age.max} ans`,
    },
  ],
  height: [
    {
      validate: (value: unknown, allValues: Record<string, unknown>) => typeof value === 'number' && value >= VALIDATION_LIMITS.height.min && value <= VALIDATION_LIMITS.height.max,
      message: `La taille doit être entre ${VALIDATION_LIMITS.height.min} et ${VALIDATION_LIMITS.height.max} cm`,
    },
  ],
  weight: [
    {
      validate: (value: unknown, allValues: Record<string, unknown>) => typeof value === 'number' && value >= VALIDATION_LIMITS.weight.min && value <= VALIDATION_LIMITS.weight.max,
...
const ProfileForm: React.FC<ProfileFormProps> = ({ onSave, onCancel, initialData }) => {
  ...
  const {
    ...
    clearErrors,
    clearFieldErrors,
  } = useFormValidation(profileValidationSchema, formData as unknown as Record<string, unknown>);
  ...
  const handleInputChange = useCallback((field: keyof ProfileData, value: string | number) => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    // Validation complète
    const validationResult = validateForm(formData as unknown as Record<string, unknown>);
  ...
  return (
    <Container size="lg" py="xl">
      <Stack gap="xl">
        <Title order={1} id="profile-title">👤 Profil Santé</Title>
            ...
            <Stack gap="lg">
              ...
              <Group justify="space-between">
                ...
                <Button 
                  ...
                  disabled={profileSaveMutation.isPending}
                >
                  {profileSaveMutation.isPending ? 'Sauvegarde...' : 'Sauvegarder'}
                </Button>
              </Group>
            </Stack>
          </form>
